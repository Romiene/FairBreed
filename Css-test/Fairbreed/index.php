...
if($pagina == 'kennels'){
    $dbh = new PDO('mysql:host='.$server.';dbname=fairbree_fairbreed;charset=utf8', $user, $pass);
    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $dbh->prepare("SELECT id, naam, CONCAT(LEFT(bio, 25), '...') AS bio, img FROM kennelhouders");    
    $stmt->execute();
...
if($pagina == 'ouderdieren'){
    $dbh = new PDO('mysql:host='.$server.';dbname=fairbree_fairbreed;charset=utf8', $user, $pass);
    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $dbh->prepare("SELECT o.naam AS naam, o.ras AS ras, o.leeftijd AS leeftijd, o.img AS img, k.id AS id FROM ouder_honden AS o JOIN kennelhouders AS k ON o.kennelhouder = k.id");    
    $stmt->execute();
    $profielen = $stmt->fetchAll(PDO::FETCH_ASSOC);                        
    $dbh = null;
    $stmt = null;
...
if($pagina == 'pups'){
    $dbh = new PDO('mysql:host='.$server.';dbname=fairbree_fairbreed;charset=utf8', $user, $pass);
    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $dbh->prepare("SELECT p.naam AS naam, p.ras AS ras, p.leeftijd AS leeftijd, p.img AS img, k.id FROM pups AS p JOIN ouder_honden AS o ON p.ouder_hond = o.id JOIN kennelhouders AS k ON o.kennelhouder = k.id");    
    $stmt->execute();
    $profielen = $stmt->fetchAll(PDO::FETCH_ASSOC);                        
    $dbh = null;
    $stmt = null;
...
<body>
    <div id="container">
        ...
        <?php
            if(isset($_GET['submenu']) && $_GET['submenu'] == 'yes'){
                include 'submenu.php';
            }
        ?>
        <main>
                ...
                if($pagina == 'mijnkennel'){
                    ...
                    foreach($profielen as $profiel){
                       echo '<div class="profiel-lijst">
                                    ...
                                    <div class="'.$pagina.'">
                                            <a href="https://fairbreed.nl/keurmerk_houders/kennels/'.$profiel['id'].'/">
                    ...
                    foreach($profielen as $profiel){
                        ...
                        echo '<div class="profiel-lijst">
                                        ...
                                        <div class="'.$pagina.'">
                                                 <a href="https://fairbreed.nl/keurmerk_houders/kennels/'.$profiel['id'].'/"><img src="'.$profiel['img'].'" height="150" width="150" class="pup-foto">
                                                <section class="pup-info">
                                                        <p class="pup-nummer">'.$profiel['naam'].'</p>
